...
What we did here was we created a function that receives the `type` of contentlet and returns a specific React Component based on that string, which could be `ContentGeneric`, `SimpleWidget` or `Event`, at least for this page. Then we render that component and pass the contentlet so we can render any of the content we need.
...
The [headless capabilities](https://dotcms.com/product/features/headless-cms) of dotCMS allows you to render content in a Single Page App, and non-dev users can control the layout of each page. The developer can jump in and create web apps without too much hassle.
