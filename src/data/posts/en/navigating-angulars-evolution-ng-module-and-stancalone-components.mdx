...
Angular offers two methods for creating self-contained components those encapsulating all their dependencies (components, services, pipes and pthers). You can either use `@NgModules` or standalone components. Understanding both is vital for working with all Angular projects, old and new.


## Before: Using `@NgModules`
In the old approach, Angular webapps werw built using `@NgModules`, which works as containers to group dependencies that needs to work together for a very specific purpose. 
